    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            <div className="mb-10 text-center">
                <h1 className="text-3xl font-bold text-text-main mb-3">Information Hub</h1>
                <p className="text-text-muted">Stay connected with essential updates, market trends, and emergency resources.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {/* Announcements */}
                        <section>
                            <h2 className="text-2xl font-bold text-text-main mb-6 flex items-center gap-2 border-b border-gray-100 pb-2">
                                <Megaphone className="text-brand h-6 w-6" /> Village Announcements
                            </h2>
                            {announcements.length === 0 ? (
                                <p className="text-text-muted italic">No recent announcements.</p>
                            ) : (
                                        ...
                                        <Card key={ann.id} className={`border-l-4 ${ann.is_pinned ? 'border-l-blue-500 bg-blue-50/30' : 'border-l-brand'}`}>
                                            <CardContent className="p-5">
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <span className="text-xs text-text-muted whitespace-nowrap bg-surface-muted px-2 py-1 rounded">
                        ...
                        {/* Emergency Contacts */}
                        <section>
                            <h2 className="text-2xl font-bold text-text-main mb-6 flex items-center gap-2 border-b border-gray-100 pb-2 mt-12">
                                <PhoneCall className="text-red-500 h-6 w-6" /> Emergency Contacts
                            </h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {contacts.length === 0 ? (
                                    <p className="text-text-muted italic sm:col-span-2">No emergency contacts listed.</p>
                                ) : (
                                    contacts.map(contact => (
                                        <div key={contact.id} className="bg-red-50 border border-red-100 p-4 rounded-xl flex flex-col justify-center">
                                            ...
                                            <a href={`tel:${contact.phone_number}`} className="text-2xl font-black text-red-600 my-2 hover:underline">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-surface border border-gray-100 rounded-2xl p-6 shadow-sm sticky top-24">
                            <h2 className="text-xl font-bold text-text-main mb-6 flex items-center gap-2">
                                <TrendingUp className="text-brand h-5 w-5" /> Local Market Prices
                            </h2>

                            <p className="text-xs text-text-muted mb-4">Prices act as a reference for buyers and sellers.</p>

                            {prices.length === 0 ? (
                                <p className="text-text-muted italic text-sm">No market prices available right now.</p>
                            ) : (
                                <div className="space-y-0 divide-y divide-gray-100 text-sm">
                                    {prices.map(price => (
                                        <div key={price.id} className="py-3 flex justify-between items-center group">
                                            <div>
                                                <div className="font-medium text-text-main group-hover:text-brand transition-colors">{price.crop_name}</div>
                                                <div className="text-xs text-text-muted">{new Date(price.price_date).toLocaleDateString()}</div>
                                            </div>
                                            <div className="text-right">
                                                ...
                                                <div className="flex justify-end mt-1">
                                                    ...
                                                    {price.trend === 'down' && <span className="text-red-500 flex items-center text-xs font-medium"><TrendingDown size={12} className="mr-0.5" /> Down</span>}
